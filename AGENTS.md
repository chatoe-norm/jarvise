...
- P3 exchange read-only (next after paper/web harden): venue-agnostic interface with Binance as the first adapter; spot balances only (not futures); live fetch on `/analytics` plus optional CLI; no order placement; API keys stay in VPS `.env` only.
